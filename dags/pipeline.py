...
@dag (schedule="@daily", start_date = datetime(2025,7,20), tags= ['Spotify_V2'],
      default_args = {
          "retries": 3,
...
def ingestion_dag():
    ...
    def prepare_duck_db():
        ...
        con.execute(f'SET s3_endpoint="{conn.extra_dejson.get("endpoint_url").replace("http://localhost:9000", "localhost:9000")}";')
    ...
    def load_files_to_duck_db(json_files, table_name):
        ...
        con.execute(f"SET s3_url_style='path';")
        con.execute(f"SET s3_secret_access_key='{conn.password}';")
        con.execute(f'SET s3_endpoint="{conn.extra_dejson.get("endpoint_url").replace("http://minio:9000", "minio:9000")}";')
        con.execute("SET s3_use_ssl=false;")
        table_name = "streaming_history"
        
        for file in json_files:
            print(file)
            con.execute(f"INSERT INTO {table_name} SELECT * FROM read_json_auto('s3://raw-spotify-data/{file}');")
    ...
    dbt_group = DbtTaskGroup(
        ...
        project_config=ProjectConfig(
            dbt_project_path="/opt/airflow/dbt/Spotify_V2/"
        ),
